...
async fn main() -> Result<()> {
    ...
    let level_package = Arc::new(
        build_project_in_dir(Path::new("../contracts/level-account"), true)
            .context("Failed to build level account contract")?,
    );

    let note_package = Arc::new(
        build_project_in_dir(Path::new("../contracts/level-note"), true)
            .context("Failed to build level note")?,
    );
    ...
    init_storage_data
        ...
        .context("Failed to seed level storage")?;

    let level_cfg = AccountCreationConfig {
        init_storage_data,
        ..Default::default()
    };

    let level_account =
        create_account_from_package(&mut client, level_package.clone(), level_cfg)
            .await
            .context("Failed to create level account")?;

    println!("Level account ID: {:?}", level_account.id().to_hex());
    ...
    let level_note = NoteBuilder::new(sender_account.id(), client.rng())
        .package((*note_package).clone())
        .tag(0)
        .build()
        .context("Failed to create level note")?;

    println!("Level note hash: {:?}", level_note.id().to_hex());

    let publish_request = TransactionRequestBuilder::new()
        .own_output_notes(vec![level_note.clone()])
        .build()
