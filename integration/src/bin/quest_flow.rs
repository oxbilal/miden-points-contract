...
async fn main() -> Result<()> {
    ...
    let quest_package = Arc::new(
        build_project_in_dir(Path::new("../contracts/quest-account"), true)
            .context("Failed to build quest account contract")?,
    );

    let note_package = Arc::new(
        build_project_in_dir(Path::new("../contracts/quest-note"), true)
            .context("Failed to build quest note")?,
    );
    ...
    init_storage_data
        ...
        .context("Failed to seed quest storage")?;

    let quest_cfg = AccountCreationConfig {
        init_storage_data,
        ..Default::default()
    };

    let quest_account =
        create_account_from_package(&mut client, quest_package.clone(), quest_cfg)
            .await
            .context("Failed to create quest account")?;

    println!("Quest account ID: {:?}", quest_account.id().to_hex());
    ...
    let quest_note = NoteBuilder::new(sender_account.id(), client.rng())
        .package((*note_package).clone())
        .tag(0)
        .build()
        .context("Failed to create quest note")?;

    println!("Quest note hash: {:?}", quest_note.id().to_hex());

    let publish_request = TransactionRequestBuilder::new()
        .own_output_notes(vec![quest_note.clone()])
        .build()
    ...
    let publish_tx_id = client
        ...
        .await
        .context("Failed to publish quest note")?;

    println!("Note publish transaction ID: {:?}", publish_tx_id.to_hex());

    let consume_request = TransactionRequestBuilder::new()
        .input_notes([(quest_note.clone(), None)])
        .build()
        .context("Failed to build consume note transaction")?;

    let consume_tx_id = client
        .submit_new_transaction(quest_account.id(), consume_request)
        .await
        .context("Failed to consume quest note")?;

    println!("Consume transaction ID: {:?}", consume_tx_id.to_hex());
    println!("Quest completed by 1 ✅");
